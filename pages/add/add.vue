<template>
	<view class="main">
		...
		<view>
			<view class="topic">
				<view class="recommend_text1">
					<view>添加话题</view>
				</view>
				<view class="topic_one_container">
					<text v-for="(item, index) in picker" :key="index" class="topic_one" @click.stop="click_topic"
						:style="{backgroundColor: (item==is_picker) ?'#fad396':''}" :data-text="item">
						#{{item}}
					</text>
				</view>
			</view>
		...
		<view class="local">
			<view class="recommend_text2">
				<view>添加地点</view>
			</view>
			<view class="local_one_container">
				<text v-for="(item, index) in local_info" :key="index" class="local_one" @click.stop="click_site"
					:style="{backgroundColor:item.is_click?'#fad396':'#00000000'}"
					:data-text="item.text">#{{item.text}}</text>
			</view>
		</view>
		<view class="imageContainer">
			<image v-for="(item, index) in picture" :key="id" :src="item" @click.stop="preview_img" :id="item"
				v-for-index="index" @load="set_title_img_size" :data-index="index" class="nowImage"></image>
			<image v-if="img_is_no_full" src="http://travel.rykerfeng.cn/wechatsoft/drawable/add_img.png"
				@click.stop="choose_img" class="add_img"></image>
		</view>
	...
	export default {
		...
		methods: {
			...
			add_to_mysql(server_img_path, user_id, title, text) {
				...
				let location = this.user_info.location
				...
				this.$api.change_mysql(JSON.parse(server_img_path).data, user_id, title, text, longitude, latitude, location, mark_menu)
					.then((res) => {
						// console.log(res)
						if (res.code == '0') {
